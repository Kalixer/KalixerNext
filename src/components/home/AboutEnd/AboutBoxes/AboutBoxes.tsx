...
import styles from './AboutBoxes.module.sass'
...
export function AboutBoxes() {
    return (
        <>
            <div className={styles.AboutBoxes}>
                {(aboutBoxesArr.map((box) => (
                    <Link href={box.link} onClick={() => scrollToTop()}>
                        <div className={styles.AboutBoxes__box}>
                            <figure>
                                <img src={box.image} alt={box.title} />
                            </figure>
                            <div className={styles.AboutBoxes__box_text}>
                                <h3>{box.title}</h3>
                                <p>{box.description}</p>
                            </div>
                        </div>
